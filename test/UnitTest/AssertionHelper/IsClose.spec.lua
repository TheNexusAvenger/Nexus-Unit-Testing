...
return function()
    ...
    describe("The FormatTestEZMessage function", function()
        it("should return nil for close values.", function()
            expect(IsClose.FormatTestEZMessage(1, 1, 0.1, "CLOSE")).to.equal(nil)
            expect(IsClose.FormatTestEZMessage(1, 2, 0.1, "NOT_CLOSE")).to.equal(nil)
        end)

        it("should return for invalid comparisons.", function()
            expect(IsClose.FormatTestEZMessage(1, Vector3.new() :: any, 0.1, "CLOSE")).to.equal("Expected value \"0, 0, 0\" (Vector3) and actual value \"1\" (number) can't be checked for closeness.")
            expect(IsClose.FormatTestEZMessage("Test", "Test", 0.1, "CLOSE")).to.equal("Expected value \"Test\" (string) and actual value \"Test\" (string) can't be checked for closeness.")
        end)

        it("should return comparisons for numbers.", function()
            expect(IsClose.FormatTestEZMessage(1, 2, 0, "CLOSE")).to.equal("Expected value to be near 2.000000 (within 0.000000) but got 1.000000 instead.")
            expect(IsClose.FormatTestEZMessage(1, 2, 3, "NOT_CLOSE")).to.equal("Expected value to not be near 2.000000 (within 3.000000) but got 1.000000 instead.")
        end)

        it("should return comparisons for datatypes.", function()
            expect(IsClose.FormatTestEZMessage(Vector3.new(1, 2, 3), Vector3.new(1, 3, 5), 0.5, "CLOSE")).to.equal("Expected value to be near \"1, 3, 5\" (Vector3) within 0.500000 but got \"1, 2, 3\" (Vector3) instead (keys Y, Z incorrect).")
            expect(IsClose.FormatTestEZMessage(Vector3.new(1, 2, 3), Vector3.new(1, 2, 3), 0.5, "NOT_CLOSE")).to.equal("Expected value to not be near \"1, 2, 3\" (Vector3) within 0.500000 but got \"1, 2, 3\" (Vector3) instead.")
        end)
    end)
end